...
public class TestBase {
	...
	public WebDriver launchBrowser(String browserName) {
		...
		/*
		 * public void takeScreenShot(String filePath){ // take screenshot // save
		 * screenshot in reports screenshot folder // add the screenshots in the reports
		 * File srcFile = ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE);
		 * try { // get the dynamic folder name FileUtils.copyFile(srcFile, new
		 * File(filePath)); //test.addScreenCaptureFromPath("path of image", "xxxx");
		 * 
		 ...
		 * }
		 * 
		 * public void getElementScreenshot(WebElement ele, String filePath) { // Get
		 * entire page screenshot File screenshot =
		 * ((TakesScreenshot)driver).getScreenshotAs(OutputType.FILE); BufferedImage
		 * fullImg; try { fullImg = ImageIO.read(screenshot); // Get the location of
		 * element on the page Point point = ele.getLocation();
		 * 
		 * // Get width and height of the element int eleWidth =
		 * ele.getSize().getWidth(); int eleHeight = ele.getSize().getHeight();
		 * 
		 * // Crop the entire page screenshot to get only element screenshot
		 * BufferedImage eleScreenshot= fullImg.getSubimage(point.getX(), point.getY(),
		 * eleWidth, eleHeight); ImageIO.write(eleScreenshot, "png", screenshot);
		 * 
		 * // Copy the element screenshot to disk File screenshotLocation = new
		 * File(filePath); FileUtils.copyFile(screenshot, screenshotLocation); } catch
